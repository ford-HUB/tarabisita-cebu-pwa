...
import TouristMenuItemDetailModal from '../../tourist/explore/modals/TouristMenuItemDetailModal.jsx'
import TouristCatalogSearchField from '../../tourist/search/TouristCatalogSearchField.jsx'
import {
  fetchPublicBusinessById,
  postGuestCatalogSearchRank,
...
import { buildCartPayloadFromMenuItem } from '../../../shared/utils/guestCartMenuItem.utils.js'
 ...
 *   cornerLink?: { href: string, label: string },
 *   guestCart?: boolean,
 *   emptyQueryHint?: string,
 *   noQueryHint?: string,
 *   noResultsHint?: string,
 *   searchInputName?: string,
 *   guestCatalog?: boolean
 * }} props
 */
const CatalogSearchResultsView = (props) => {
  const {
    backLink,
    cornerLink,
    guestCart = false,
    guestCatalog = false,
    emptyQueryHint = 'Add a search from the home hero to see matching menu items and packages here.',
    noQueryHint = 'Type a keyword above and press Explore, or use trending chips — results update from dishes and stay packages in our catalog.',
    noResultsHint = 'No catalog items matched that search. Try different keywords.',
    searchInputName = 'catalog-search-results'
  } = props
  ...
  useEffect(() => {
    void loadMenuFeed('ALL', { guest: guestCatalog })
  }, [loadMenuFeed, guestCatalog])
  ...
  const rankCatalogSearch = guestCatalog ? postGuestCatalogSearchRank : postTouristCatalogSearchRank
  ...
  useEffect(() => {
    ...
    timer = setTimeout(() => {
      ...
      void rankCatalogSearch({ query: qRaw, items: head })
        .then((res) => {
          if (cancelled) return
  ...
  }, [rankFingerprint, qRaw, needle, rankCatalogSearch])
  ...
  const handleAddToCartClick = useCallback(
    (e, item) => {
      ...
      e.stopPropagation()
      if (!guestCart) return
      const payload = buildCartPayloadFromMenuItem(item)
      if (!payload) return
      addGuestItem(payload)
    },
    [guestCart, addGuestItem]
  )

  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 px-6 py-10 md:space-y-8 lg:px-10">
          ...
          {qRaw ? (
            <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[#5b5b5b]">
              ...
              <span>
                ...
                resort and hotel stay packages, and other catalog items.
              </span>
            </p>
          ) : (
            <p className="mt-2 max-w-2xl text-sm text-[#5b5b5b]">{emptyQueryHint}</p>
          )}
        </div>
        {cornerLink ? (
          <Link
      ...
      {!listBlockingLoading && qRaw && !rankedResults.length ? (
        <p className="rounded-xl border border-[#e7dfd5] bg-[#fbf9f6] p-6 text-sm text-[#5b5b5b]">{noResultsHint}</p>
      ) : null}

      {!listBlockingLoading && !qRaw ? (
        <p className="rounded-xl border border-dashed border-[#e7dfd5] bg-white p-6 text-sm text-[#5b5b5b]">{noQueryHint}</p>
      ) : null}
            ...
            {displayResults.map((item) => {
              ...
              return (
                <li key={catalogItemKey(item)}>
                  <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[#e7dfd5] bg-[#f8f5f0] shadow-sm transition hover:border-[#d4c4b6] hover:shadow-md">
                    <button
                      type="button"
                    ...
                    {showAvailable && guestCart ? (
                      <div className="border-t border-[#e7dfd5] px-3 pb-3 pt-2">
                        <button
                          type="button"
                          onClick={(e) => handleAddToCartClick(e, item)}
                          className="w-full rounded-full border border-[#e7dfd5] bg-white py-2 text-xs font-semibold text-[#9b5a2c] transition hover:border-[#ff7a1a] hover:text-[#ff7a1a]"
                        >
                          Add to cart
                        </button>
                      </div>
...
export default CatalogSearchResultsView
